...
function setReadCB(messages)
{
	if(messages.length===1 && selectedService)
	{
		...
		selectedService.messageStorage.updateMessages([messages[0]],updateMsgCB);
	}
}
...
function deleteMailRequestByMailIds(mailIDs) {
	...
	else 
	{
		...
		try {
				...
				var filter = new tizen.AttributeFilter("folderId", "EXACTLY",
						currentFolder);
